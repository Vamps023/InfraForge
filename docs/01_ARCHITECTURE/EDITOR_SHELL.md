...
- Each operation carries a stable ID, name, type, state, progress (when provided), processed/total work (when available, preserved as `bigint` to avoid `uint64` precision loss), diagnostics/error state, source, and target canonical ID.
...
- **Backend event ingestion is implemented.** The engine emits `job.queued`/`job.started`/`job.progress`/`job.completed`/`job.failed` events from the canonical `CommandProcessor` execution path. The `shellEventProjector.ts` module subscribes to the engine event stream and projects these events into the operations store. Optional `progress`/`processed`/`total` fields use protobuf `optional` semantics (absent vs. zero is distinguishable). `uint64` counts are preserved as `bigint` throughout the projection — no `Number()` round-trip. Terminal states (completed/failed/cancelled) are protected against out-of-order non-terminal events. Duplicate event delivery is idempotent via stable job ID upserts.
...
- **Backend diagnostic event ingestion is implemented.** The engine emits `diagnostic.added`/`diagnostic.removed`/`diagnostic.cleared` events from the canonical `CommandProcessor` execution path. The `shellEventProjector.ts` module subscribes to the engine event stream and projects these events into the problems store. The current diagnostic source is the vertical-reference georeference check: when a project is created/opened without a vertical CRS, a `diagnostic.added` event is emitted; when a vertical CRS is added via `set_georeference`, a `diagnostic.removed` event is emitted; when the project closes, a `diagnostic.cleared` event is emitted. Shell-owned diagnostics (engine-session, viewport) survive a backend `diagnostic.cleared` with empty source. Duplicate event delivery is idempotent via stable diagnostic ID upserts.
...
- No real domain operation currently supplies progress. The engine emits `job.queued`/`job.started`/`job.completed`/`job.failed` for every command, but `job.progress` is not emitted because no current command produces real progress increments. Progress events will be emitted when a domain operation (terrain import, mesh generation, etc.) genuinely has progress to report.
- Cancellation is modeled in the protocol (`JobCancelledEvent`) but not wired end-to-end because the engine does not implement a cancellation command. `cancellable` is `false` for all current operations.
- The event stream does not provide replay cursors or server-side event history. Reconnect after a disconnect does not replay missed events. Idempotency is handled via stable job/diagnostic ID upserts so duplicate delivery is harmless, but events missed during a disconnect are not recovered. A future replay mechanism (event sequence numbers, cursor-based replay) would be needed for full reconnect safety.
- Diagnostics currently cover only the vertical-reference georeference condition. As more canonical domain conditions are implemented (terrain validation, road geometry checks, etc.), additional diagnostic sources will be added.
