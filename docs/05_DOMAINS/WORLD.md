...
| `domain::world::ChunkGrid` (`ChunkGrid.hpp`/`.cpp`) | Logical chunk cells (`ChunkCoord`), configurable `ChunkGridConfig` (default `defaultChunkSize` = 1000 in the canonical linear unit, defined once), floor-division mapping, conservative closed-bounds enumeration, cell footprints. |
| `domain::world::Invalidation` (`Invalidation.hpp`/`.cpp`) | `InvalidationClass` (geometry, material, topology, terrain, simulation) and the composable `InvalidationMask` bit set. Strongly typed — never free-form strings. |
| `domain::world::SpatialIndex` (`SpatialIndex.hpp`/`.cpp`) | Canonical sparse index: entity id → canonical bounds → occupied cells. `insert`/`update`/`remove` return an `IndexMutation` carrying previous cells, updated cells, their deterministic union (dirty), the declared invalidation classes, and the new index revision. `ChunkDirtySet` accumulates per-cell class masks across mutations — the one canonical chunk-diff mechanism. |
...
| `domain::world::ChunkCacheMetadata` (`ChunkCacheMetadata.hpp`) | Versioned provenance record (schema version, generator revision, source revision) with `isCurrent` staleness checking. Caches are always reconstructable from canonical data. |
...
- Bounds edges are closed: `contains` is `min <= v <= max`, rectangles
  ...
  extent.
- Coordinate mapping is rejected (`CoordinateOutOfRange`) beyond the
  exactly representable double range (±2^53 cells) or when a bounds spans
  more than `ChunkGrid::maxEnumeratedChunks` cells.
...
generator revision, and the canonical source revision it was derived from;
any drift marks the cache stale for rebuild. Caches are never canonical
storage — no entity exists only because a chunk cache contains it.
...
- `engine/tests/SpatialBoundsTests.cpp` — empty/point/closed-edge
  ...
  finiteness.
- `engine/tests/ChunkGridTests.cpp` — default and configurable sizes,
  floor division and exact-boundary behavior, single/multi-cell and
  negative enumeration, footprint round-trip, invalid size and
  unrepresentable-coordinate rejection, enumeration cap.
