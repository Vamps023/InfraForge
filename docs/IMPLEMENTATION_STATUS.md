...
| Native dependency manifest | Verified | `vcpkg.json` (ixwebsocket, protobuf, sqlite3, nlohmann-json, doctest) pinned to immutable registry baseline; full native build passed on Windows and Linux in the native workflow for commit `28d46b877c8cc30759e9f653876c3afa226c8056` |
...
| World validation | Partially implemented; verified | GitHub issue #14; `engine/src/application/validation/` — canonical diagnostic model with identity (code + entities), validator interface/registry with non-empty/duplicate name rejection, `ValidationService` use case with revision snapshot and cooperative cancellation (partial results discarded), `DiagnosticStore` engine-owned lifecycle with diff/broadcast of added/removed/cleared events, `ProjectFoundationValidator` checking real canonical state; typed `world.check` + `world.cancel_check` protocol wired into command/result/event envelopes; frontend projection with identity-based add/remove, stale/cancelled state, Problems panel integration; native doctest suite covers deterministic output, stable codes, severity/source/revision, project-not-open, validator failure isolation, cancellation before/during execution, partial-result policy, diagnostic identity, added/removed/clear events, project close invalidation, no duplicate events; native build, tests, proto lint, TypeScript typecheck, and frontend build all pass; **entity selection/framing not yet implemented** — no frameable entity type exists until renderer (Issue #2) and domain entities merge; **revision_changed invalidation wired but not triggered** — no current mutation path changes the revision |
