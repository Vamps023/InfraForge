...
| Terrain interaction | `src/engine/roadSurfaceInfluence.ts` and frontend workflows | `ConformRoadToTerrain` command through `TerrainService::sample` and canonical elevation updates | ADAPT — IMPLEMENTED | Donor workflow intent was adapted as an explicit native road command. It samples canonical project-space terrain, rejects any NoData/outside-coverage station atomically, applies an explicit surface offset, and writes the normal persisted/undoable elevation profile. No renderer or frontend geometry mutation is involved. |
...
5. ~~Add an explicit road-to-terrain conformance command.~~ Implemented through
   canonical Terrain sampling and Road elevation history. Next, adapt OSM/GIS
   import only after the remaining runtime acceptance gate is proven.
