...
| Adaptive alignment sampling | `AlignmentSampler.cpp::adaptiveStations`, `RoadGeometry.cpp::adaptiveStepForRadius` | fixed error/step logic in `RoadTessellation.cpp` | ADAPT | Donor contains useful curvature/grade/banking-driven sampling ideas. Port only after replacing heuristic thresholds with an explicit chord/vertical/bank error contract and regression tests. |
...
| Width/cross-section | `RoadProfile.h`, `LaneSectionEvaluator.cpp`, `LaneLayout.cpp` | not yet canonical | ADAPT | Donor station-aware element widths/tapers and lateral ranges are the strongest reusable road gap. Port into a new engine-owned cross-section model attached to Road IDs, with protocol, SQLite migration, undo/redo, tessellation, and tests. Do not port donor global/profile ownership. |
...
| Banking/superelevation | `AlignmentSampler.cpp`, `RoadMeshBuilder.cpp` | canonical `SuperelevationProfile`, current ribbon tessellation | ADAPT | InfraForge already persists banking but should apply it to the cross-section frame. Donor evaluation and mesh-frame behavior provide a useful implementation reference; add unit tests for sign convention and rotated edge elevations. |
...
3. Add an engine-owned Road cross-section/width model, adapting the donor's
   station-aware lateral layout and mesh-strip concepts through InfraForge's
   protocol, persistence, undo/redo, chunk tessellation, and RoadPass.
4. Apply canonical superelevation to that cross-section frame and add adaptive
   chord-error tessellation regression fixtures.
5. Adapt OSM import only after the canonical width/profile mesh path is proven.
